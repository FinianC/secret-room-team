...
public class StringUtil extends TextUtils {
     ...
     * @return
     */
    public static String joinIntegerArr(Integer[] arr,String splitStr){
        if(arr.length==0) return "";
        StringBuffer joinStr=new StringBuffer();
        for (int i=0;i<arr.length;i++){
            joinStr.append(arr[i]+splitStr).toString();
        }
      String  outJoinStr=joinStr.substring(0,joinStr.length()-1);
        return outJoinStr;
    }
     ...
     * @return
     */
    public static String createOrderNo(String prefix,int length) {
        String orderNo = "";
